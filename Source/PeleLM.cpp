...
#pragma omp parallel
#endif
  {
    for (MFIter S_mfi(Smf,true); S_mfi.isValid(); ++S_mfi)
    {
      ...
      {
        for (int comp=0; comp < nspecies; comp++){
          edgestate[d][S_mfi].plus<RunOn::Host>(edgestate[d][S_mfi],comp+1,0,1);
          edgeflux[d][S_mfi].plus<RunOn::Host>(edgeflux[d][S_mfi],comp+1,0,1);
        }
      }
    }
  }
...
#endif
  {
    ...
    for (MFIter S_mfi(Smf,true); S_mfi.isValid(); ++S_mfi)
    {
      for (int d=0; d<AMREX_SPACEDIM; ++d)
      {
        ...
        eR.resize(ebox,1);
        eR.copy<RunOn::Host>(edgestate[d][S_mfi],0,0,1);
        eR.invert<RunOn::Host>(1.0,ebox,0,1);

        eY.resize(ebox,nspecies);
        eY.copy<RunOn::Host>(edgestate[d][S_mfi],1,0,nspecies);
        ...
        edgestate[d][S_mfi].copy<RunOn::Host>(eH,ebox,0,ebox,nspecies+1,1);      // Copy H into estate
        edgestate[d][S_mfi].mult<RunOn::Host>(edgestate[d][S_mfi],ebox,0,nspecies+1,1); // Make H.Rho into estate

        // Copy edgestate into edgeflux. ComputeAofs() overwrites but needs edgestate to start.
        edgeflux[d][S_mfi].copy<RunOn::Host>(edgestate[d][S_mfi],ebox,nspecies+1,ebox,nspecies+1,1);
      }
    }
  }
...
#endif
  {
    for (MFIter S_mfi(Smf,true); S_mfi.isValid(); ++S_mfi)
    {
      for (int d=0; d<AMREX_SPACEDIM; ++d)
      {
        ...
        (*EdgeState[d])[S_mfi].copy<RunOn::Host>(edgestate[d][S_mfi],efbox,0,efbox,Density,nspecies+1);
        (*EdgeState[d])[S_mfi].copy<RunOn::Host>(edgestate[d][S_mfi],efbox,nspecies+1,efbox,RhoH,1);
        (*EdgeState[d])[S_mfi].copy<RunOn::Host>(edgestate[d][S_mfi],efbox,nspecies+2,efbox,Temp,1);
        (*EdgeFlux[d])[S_mfi].copy<RunOn::Host>(edgeflux[d][S_mfi],efbox,0,efbox,Density,nspecies+1);
        (*EdgeFlux[d])[S_mfi].copy<RunOn::Host>(edgeflux[d][S_mfi],efbox,nspecies+1,efbox,RhoH,1);
        (*EdgeFlux[d])[S_mfi].copy<RunOn::Host>(edgeflux[d][S_mfi],efbox,nspecies+2,efbox,Temp,1);
      }
    }
  }
...
#endif
{
  ...
  for (MFIter mfi(Ssync,true); mfi.isValid(); ++mfi)
  {
    ...
    for (int d=0; d<AMREX_SPACEDIM; ++d)
    {
      ...
      efab[d].mult<RunOn::Host>(be_cn_theta,ebox,0,nspecies);
    }
    ...
    // Ssync = "RHS from diffusion solve" + (dt/2) * div (delta gamma)
    Ssync[mfi].copy<RunOn::Host>(update,box,0,box,first_spec-BL_SPACEDIM,nspecies);
  }
}
